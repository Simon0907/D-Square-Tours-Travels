...
const FamousPackageEnquiry = () => {
  ...
  const handleVehicleChange = (e) => {
    ...
    if (vehicle) {
      ...
      setTotalAmount(rentCost + kmCost + EXTRA_CHARGES);
    } else {
  ...
  // Min return date = day after travel date
  ...
  const handleSubmit = (e) => {
    ...
    const booking = {
      id:              `FAM-${Date.now()}`,
      type:            "Famous Package",           // ← tag for admin to separate
      packageTitle:    "5 Days Round Trip",
      ...
      travelDate:      form.travelDate,            // ← stored
      travelTime:      form.travelTime,            // ← stored
      returnDate:      form.returnDate,            // ← stored
    ...
    // ── Save to its own key so admin can show separately ─────────────────────
    const existing = JSON.parse(localStorage.getItem("adminFamousBookings")) || [];
    existing.unshift(booking);
    localStorage.setItem("adminFamousBookings", JSON.stringify(existing));

    // Also push into general adminBookings so dashboard total counts it
    const all = JSON.parse(localStorage.getItem("adminBookings")) || [];
    all.unshift(booking);
    localStorage.setItem("adminBookings", JSON.stringify(all));
      ...
      <div className="fpe-container">
        ...
        <form onSubmit={handleSubmit} className="fpe-form">
          ...
          <div className="fpe-section">
            ...
            {form.travelDate && form.travelTime && form.returnDate && (
              <div className="fpe-date-summary">
                <span>📅</span>
                <span>
                  Pickup on <strong>{fmt(form.travelDate)}</strong> at <strong>{form.travelTime}</strong>
                  &nbsp;→ Return on <strong>{fmt(form.returnDate)}</strong>
                </span>
              </div>
            )}
          </div>
          ...
          <div className="fpe-actions">
            ...
            <button type="submit" className="fpe-submit">
              Confirm Booking ✓
            </button>
          </div>
